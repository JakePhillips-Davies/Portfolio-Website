...
export class FileSys {
    ...
    //░▀▀▀░░▀░░▀░▀░░▀░░▀▀▀░▀▀▀░▀▀▀
    //
    static DirectoryForEach(directory_, function_) {
        directory_.children.forEach(directory => {
            function_(directory);
        });
    }
    static DirectoryForEachRecursive(directory_, function_) {
    ...
    static GetDirectoryByExactPath(path_) {
        let resultDir = null;
        let path = path_.toLowerCase();
        function search(directory_) {
            // check if still on the right path
            if (path.includes(directory_.path.toLowerCase()) == false) {
                return;
            }
